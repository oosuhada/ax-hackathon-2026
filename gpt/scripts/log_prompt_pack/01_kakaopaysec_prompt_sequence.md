# 카카오페이증권 — AX 해커톤 Codex 플러그인 로그용 프롬프트 시퀀스

> 사용법: 아래 `PROMPT 01`부터 실제 AI 세션에 하나씩 복사해 입력하세요.  
> AI의 실제 답변까지 포함된 전체 대화가 최종 `logs/kps-investor-risk-brief_raw_log.md`가 됩니다.  
> 이 문서는 “프롬프트 대본”이지, 최종 제출용 원본 로그가 아닙니다.

## 프로젝트 요약

- 대상 기업: **카카오페이증권**
- 플러그인 이름: **`kps-investor-risk-brief`**
- 핵심 문제: AI 투자정보 콘텐츠가 초보 투자자에게 투자 권유처럼 오해되거나 핵심 리스크·출처·불확실성을 충분히 전달하지 못할 수 있는 문제
- 주 사용자: 카카오페이증권의 AI 투자정보/콘텐츠/금융소비자보호 담당자
- 주요 산출물: risk_checked_summary.md, missing_risk_disclosures.md, misleading_phrases.md, plain_korean_rewrite.md, compliance_checklist.md, source_traceability_table.md
...
```text
나는 AX 인재전쟁 해커톤 예선 과제로 카카오페이증권을 대상으로 한 Codex 플러그인을 만들고 있다.
...
이번 플러그인 후보는 `kps-investor-risk-brief`이고, 내가 풀고 싶은 문제는 다음과 같다:
"AI 투자정보 콘텐츠가 초보 투자자에게 투자 권유처럼 오해되거나 핵심 리스크·출처·불확실성을 충분히 전달하지 못할 수 있는 문제"
...
4. 최종적으로 `src/skills/kps-investor-risk-brief/references/evidence_sources.md`에 넣을 수 있는 구조로 정리해줘.

검색 키워드 후보는 다음과 같다:
카카오페이증권 AI 투자정보 주식봇 어땠지, 카카오페이증권 금융소비자보호 투자위험 고지, 금융위원회 금융소비자보호 투자권유 광고 규제, 개인투자자 AI 투자정보 리스크
...
4. Why is it important for 카카오페이증권?
...
```text
이제 `kps-investor-risk-brief`의 사용자 시나리오와 입출력을 구체화해줘.
...
이 조건에 맞춰 `kps-investor-risk-brief`의 파일 구조를 설계해줘.
각 파일의 역할도 설명해줘.
...
3. 플러그인 이름은 `kps-investor-risk-brief`으로 해줘.
4. description은 카카오페이증권 대상 문제와 연결되게 작성해줘.
...
```text
이제 `src/skills/kps-investor-risk-brief/SKILL.md`에 들어갈 내용을 작성해줘.
...
```text
`kps-investor-risk-brief`을 검증하기 위한 샘플 입력 데이터를 만들어줘.
...
조건:
- 대상 기업은 카카오페이증권
- 플러그인 이름은 `kps-investor-risk-brief`
...
```text
이제 `kps-investor-risk-brief`의 검증 시나리오를 만들어줘.
...
```text
지금까지 설계한 `kps-investor-risk-brief` 제출물이 해커톤 조건에 맞는지 최종 점검해줘.

체크할 것:
1. src/.codex-plugin/plugin.json 존재
2. skills/kps-investor-risk-brief/SKILL.md 존재
...
```text
logs/kps-investor-risk-brief_raw_log.md
```
