# 삼일PwC — AX 해커톤 Codex 플러그인 로그용 프롬프트 시퀀스

> 사용법: 아래 `PROMPT 01`부터 실제 AI 세션에 하나씩 복사해 입력하세요.  
> AI의 실제 답변까지 포함된 전체 대화가 최종 `logs/samil-ax-usecase-prioritizer_raw_log.md`가 됩니다.  
...
- 핵심 문제: 기업이 생성형 AI/AX 도입 필요성을 인식해도 어떤 업무부터 자동화해야 하는지, ROI와 리스크를 어떻게 비교해야 하는지 판단하기 어려운 문제
- 주 사용자: 삼일PwC AX 컨설턴트, 기업 DX/전략/재무혁신 담당자, 중견기업 경영진
- 주요 산출물: ax_usecase_matrix.md, quickwin_candidates.md, poc_4week_plan.md, executive_onepager.md, governance_risk_checklist.md, data_readiness_questions.md
...
```text
나는 AX 인재전쟁 해커톤 예선 과제로 삼일PwC을 대상으로 한 Codex 플러그인을 만들고 있다.
...
이번 플러그인 후보는 `samil-ax-usecase-prioritizer`이고, 내가 풀고 싶은 문제는 다음과 같다:
"기업이 생성형 AI/AX 도입 필요성을 인식해도 어떤 업무부터 자동화해야 하는지, ROI와 리스크를 어떻게 비교해야 하는지 판단하기 어려운 문제"
...
4. 최종적으로 `src/skills/samil-ax-usecase-prioritizer/references/evidence_sources.md`에 넣을 수 있는 구조로 정리해줘.

검색 키워드 후보는 다음과 같다:
삼일PwC AX Node 생성형 AI use case, Samil PwC AI business use cases, 기업 생성형 AI 도입 리스크 거버넌스 ROI, 재무 업무 AI 자동화 PwC Korea
...
4. Why is it important for 삼일PwC?
...
```text
이제 `samil-ax-usecase-prioritizer`의 사용자 시나리오와 입출력을 구체화해줘.
...
이 조건에 맞춰 `samil-ax-usecase-prioritizer`의 파일 구조를 설계해줘.
각 파일의 역할도 설명해줘.
...
3. 플러그인 이름은 `samil-ax-usecase-prioritizer`으로 해줘.
4. description은 삼일PwC 대상 문제와 연결되게 작성해줘.
...
```text
이제 `src/skills/samil-ax-usecase-prioritizer/SKILL.md`에 들어갈 내용을 작성해줘.
...
```text
`samil-ax-usecase-prioritizer`을 검증하기 위한 샘플 입력 데이터를 만들어줘.
...
조건:
- 대상 기업은 삼일PwC
- 플러그인 이름은 `samil-ax-usecase-prioritizer`
...
```text
이제 `samil-ax-usecase-prioritizer`의 검증 시나리오를 만들어줘.
...
```text
지금까지 설계한 `samil-ax-usecase-prioritizer` 제출물이 해커톤 조건에 맞는지 최종 점검해줘.

체크할 것:
1. src/.codex-plugin/plugin.json 존재
2. skills/samil-ax-usecase-prioritizer/SKILL.md 존재
...
```text
logs/samil-ax-usecase-prioritizer_raw_log.md
```
