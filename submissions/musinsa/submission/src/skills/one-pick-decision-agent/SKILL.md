...
Respond strictly in JSON (ALWAYS output valid JSON, regardless of input length. Keep response well under a 1000-token limit to prevent truncation): (Do not use markdown blocks inside JSON string values. DO NOT wrap the JSON output in markdown code blocks.) Ensure `one_pick_item`, `why_this`, `confidence`, and `return_risk_note` are strictly formatted as JSON strings. `confidence` must be a string containing the percentage sign (e.g. "95%"), not a number. `rejected_options` must be a JSON array of strings. (DO NOT omit any keys from the output under any circumstances. If a field is not applicable or `one_pick_item` is "N/A", use "N/A" for `confidence` and `return_risk_note`. DO NOT mutate or rename any JSON keys). You MUST explicitly sanitize user inputs reflected in the `why_this` field by escaping or stripping all raw double quotes ("), backslashes (\), and newline/control characters to prevent JSON parsing errors.
```json
{
