...
def run_codex(session, turn, prompt):
    ...
    try:
        ...
        # When waiting for standard input, codex might print "Reading additional input from stdin..."
        # to stderr and not return a proper JSON on stdout if there's an error. 
        # So check both stderr and stdout.
...
def run_session(session_id, attack_vector, turns):
    ...
    # We already did turn 1 and 2~8 failed. So let's delete the old log and restart properly.
    ...
    for i in range(min(turns, len(prompts))):
        ...
        for attempt in range(3):
            # Sometimes codex exec resume --last needs a small delay to read the previous output file
            time.sleep(2)
