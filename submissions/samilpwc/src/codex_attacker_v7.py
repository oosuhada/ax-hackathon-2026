...
def run_codex(session, turn, prompt):
    ...
    if turn > 1:
        # Note: resume command format is codex exec resume [OPTIONS] [SESSION_ID]
        # and doesn't take -s or -o flags in the same way. We need to pass them before resume
        # actually, let's just use the basic input method without -s for resume to be safe
        cmd.extend([
            "--json",
