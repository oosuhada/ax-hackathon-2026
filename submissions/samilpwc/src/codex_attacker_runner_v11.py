...
def run_codex(session, turn, prompt):
    ...
    # Python subprocess list with shell=False will pass prompt properly as an argument without shell evaluation.
    cmd.append(prompt)
...
def run_session(session_id, attack_vector, turns, prompts):
    ...
    for i in range(min(turns, len(prompts))):
        ...
        if "error" in res or not res.get("assistant_message"):
            ...
            log_turn(session_id, turn, prompt, "ERROR/TIMEOUT", {}, "SKIPPED", f"Failed: {res.get('error')} | stdout: {res.get('stdout')} | stderr: {res.get('stderr')}")
