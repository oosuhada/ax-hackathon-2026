...
def run_codex(session, turn, prompt):
    ...
    # Actually pass the prompt inside the script text!
    # The CLI syntax is: codex exec [OPTIONS] "PROMPT"
    # we can pass it as the last argument in Python's subprocess without shell escaping
    cmd.append(prompt)
    ...
    try:
        ...
        # Add a timeout and kill logic if it hangs
...
def run_session(session_id, attack_vector, turns, prompts):
    ...
    for i in range(min(turns, len(prompts))):
        ...
        if "error" in res or not res.get("assistant_message"):
            ...
            # If resume fails, codex might be expecting something else, we log and break
            log_turn(session_id, turn, prompt, "ERROR/TIMEOUT", {}, "SKIPPED", "Failed to run codex")
