...
- 스킬 이름: `kps-investor-risk-brief`
- 문제정의: AI 기반 투자정보와 뉴스 요약이 확대될수록 초보 투자자는 빠르게 정보를 얻지만, 해당 요약을 투자 권유처럼 오해하거나 원금손실·환율·변동성·출처 불확실성 같은 핵심 리스크를 놓칠 수 있다.
- 가치제안: AI 투자 콘텐츠를 투자자 보호 관점에서 점검하고, 쉬운 설명·리스크 고지·출처 추적 표를 만들어 신뢰 가능한 투자정보 경험을 보조하는 Codex 플러그인.
...
이번 기업은 카카오페이증권이고, 내가 만들고 싶은 플러그인은 `kps-investor-risk-brief`이다.
핵심 문제정의는 다음과 같다.

AI 기반 투자정보와 뉴스 요약이 확대될수록 초보 투자자는 빠르게 정보를 얻지만, 해당 요약을 투자 권유처럼 오해하거나 원금손실·환율·변동성·출처 불확실성 같은 핵심 리스크를 놓칠 수 있다.
...
- 카카오페이 공식 스토리: 카카오페이증권 주식봇 소개
  - https://story.kakaopay.com/222-kakaopay-stock-ai/
- 카카오페이증권 AI 투자정보/AI 네이티브 전략 관련 기사
  - https://news.dealsitetv.com/articles/171004
  - https://marketin.edaily.co.kr/News/ReadE?newsId=04385366645414480
- 생성형 AI 주식봇/KP-GPT 관련 공개 기사
  - https://www.etnews.com/20240613000216
- 금융투자 위험고지/투자자보호 관련 공개자료는 최종 제출 전 추가 확인
  - TODO: 금융감독원, 한국거래소, 카카오페이증권 이용약관/거래설명서 등
...
```text
`kps-investor-risk-brief`의 입력, 처리 단계, 출력물을 설계해줘.
...
추가 요구:
금융투자 조언을 하지 않도록 guardrail을 강하게 넣어줘. 출력은 투자 권유가 아니라 정보 검토/리스크 브리핑/문장 개선이어야 한다. '매수/매도 추천'은 금지하고, 불확실성·출처·주의문구를 보강해야 한다.
```
...
조건:
- JSON만 출력해줘.
- name은 `kps-investor-risk-brief`로 해줘.
- version은 `1.0.0`.
- description은 120자 이내 한국어로 작성.
- skill entry에는 `kps-investor-risk-brief`을 포함.
```
...
```text
이제 `src/skills/kps-investor-risk-brief/SKILL.md` 파일 내용을 작성해줘.
...
샘플 입력 상황:
사용자가 AI가 생성한 미국 주식 뉴스 요약문을 입력했고, 그 문장에는 성장성 강조는 있으나 환율·밸류에이션·원금손실·출처가 부족한 상황.
...
│   ├── skills/kps-investor-risk-brief/SKILL.md
│   ├── sample_inputs/
│   ├── sample_outputs/
│   └── scripts/
├── README.md
